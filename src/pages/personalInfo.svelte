<script>
    ...
    import {
        getPersonalInfo,
        personalInfoId, currentPersonalInfo, postPersonalInfo
    ...
    let validity=false;
    $:  ($currentPersonalInfo===undefined) 
        ? console.log('noUser')
        : setValues();
    function setValues() {
        values = $currentPersonalInfo;
        currentPersonalInfo.set(undefined);
    }

    const saveInfo = async () => {
        validity = await personalInfoSchema.isValid(values);
        await console.log(validity);
        if(validity){
            if(values.personalInfoId===undefined)
                values = {"personalInfoId":$personalInfoId, ...values};
            console.log( await postPersonalInfo(values));
        }
    };
